...
def load_img(path: str | Path, background: int = 0, 
    ...
    """ Load a TPS trace image specifiec by path, convert to grayscale if
...
def tps_counts(img: NDArray[int_], m: float, q: float, x0: float, y0: float, 
               ...
               dU: float = 0.1
               ) -> tuple[NDArray[float_], NDArray[float_], MatLike] :
    ...
    x_px = x0 + x*px_dens - pinhole_sz/2
    y_px = y0 + y*px_dens - pinhole_sz/2 

    # Rotate img
    rot_mat = cv2.getRotationMatrix2D((x0, y0), rot_ang, 1)
    rot_img = cv2.warpAffine(img, rot_mat, img.shape[::-1])

    # Interpolate to get pixel values at x_rot, y_rot
    Nx = len(img[0])
    Ny = len(img)
    x_det = np.linspace(1, Nx, Nx) - x0
    y_det = np.linspace(1, Ny, Ny) - y0
    interp_img = RectBivariateSpline(y_det, x_det, rot_img)
    spec = part_cal*interp_img(y_px, x_px, grid = False)[::-1]
